...
//CRC 餘式表;//
const unsigned int CrcByteTable[256] =
{ 
...
unsigned int CalCrcByte(unsigned char *ptr, unsigned char len)
{
    ...
    while(len--!=0) 
    {
        // 以 8 位二進制數的形式暫存CRC 的高8 位 ;//
        da = (unsigned char)(crc/256);  

        // 左移 8 位，相當於CRC 的低8 位乘以28 ;//
        crc <<= 8;                      

        // 高 8 位和當前字節相加後再查表求CRC ，再加上以前的CRC ;//
        crc ^= CrcByteTable[da^*ptr];         
        ptr++;
    }
    return(crc);
}
...
unsigned int CalCrcHalfByte(unsigned char *ptr, unsigned char len) 
{
    ...
    while( len--!=0 )
    {
        ...
        // CRC 右移4 位，相當於取CRC 的低12 位）;//
        crc<<=4;       

        // CRC 的高4 位和本字節的前半字節相加後查表計算CRC，然後加上上一次 CRC 的餘數 ;//
        crc^=crc_ta[da^(*ptr/16)];        

        // 暫存 CRC 的高4 位 ;//
        da=((unsigned char)(crc/256))/16; 

        // CRC 右移4 位， 相當於 CRC 的低12 位） ;//
        crc<<=4;

        // CRC 的高4 位和本字節的後半字節相加後查表計算CRC，然後再加上上一次 CRC 的餘數 ;//
